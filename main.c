...
GET /api/history/variance?receiver_id=1&minutes=60
  → per-minute amplitude variance from the continuous aggregate

GET /api/history/snapshot?receiver_id=1&limit=1
  → raw amplitude/phase arrays for the N most recent samples
    (used to render the subcarrier heatmap in the frontend)
"""
...
async def get_snapshot(
    ...
    """Return the raw amplitude arrays for the most recent N samples.

    The frontend uses this to draw the per-subcarrier amplitude heatmap.
    """
    pool = get_pool()
    rows = await pool.fetch(
        """
